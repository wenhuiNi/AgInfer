...
int main(int argc,char** argv) {try {
  ...
  for(auto magic:{"AIANG1\0","AIGRD1\0"}) {
  ...
  auto norm=base,res=base;std::memcpy(norm.data(),"AIANG1\0",8);std::memcpy(res.data(),"AIGRD1\0",8);
  ...
  std::array<CommandBuffer,9> b{};
  for(int i=0;i<9;++i) {
    b[i].bytes=i==1?12288:102400;b[i].access=CommandOperandAccess::kRead;
  ...
  auto compare=[&] {
    for(auto pair:{std::pair{2,3},std::pair{7,8}}) {
  ...
  Check(nc->Execute(stream).ok());Check(rc->Execute(stream).ok());Check(cudaStreamSynchronize(stream)==cudaSuccess);
  compare();first=out;
  cudaGraph_t graph;cudaGraphExec_t exec;
  Check(cudaStreamBeginCapture(stream,cudaStreamCaptureModeThreadLocal)==cudaSuccess);
  Check(nc->Execute(stream).ok());Check(rc->Execute(stream).ok());
  Check(cudaStreamEndCapture(stream,&graph)==cudaSuccess);Check(cudaGraphInstantiate(&exec,graph,0)==cudaSuccess);
  for(int phase:{1,0}) {
    fill(phase);
    Check(driver.Launch(old.value(),grid.data(),block.data(),0,stream,args).ok());Check(oc->Execute(stream).ok());
    for(int i:{2,7})Check(cudaMemsetAsync(b[i].data,0xff,102400,stream)==cudaSuccess);
  ...
  for(bool is_norm:{true,false}) {
    auto payload=is_norm?norm:res;
    std::vector<CommandBuffer> bindings=is_norm?std::vector<CommandBuffer>(nb.begin(),nb.end()):std::vector<CommandBuffer>(rb.begin(),rb.end());
  ...
  nc.reset();rc.reset();oc.reset();for(auto v:b)Check(cudaFree(v.data)==cudaSuccess);
