#pragma once

#include "status.h"

#include <array>
#include <cstddef>
#include <cstdint>

namespace aginfer::internal {

inline constexpr std::size_t kKvPackPayloadSize = 192;
inline constexpr std::uint16_t kKvPackSchemaMajor = 1;
inline constexpr std::uint16_t kKvPackSchemaMinor = 0;

struct KvPackPayloadView {
  std::uint32_t target_arch = 0;
  std::array<std::uint32_t, 3> grid{0, 0, 0};
  std::array<std::uint32_t, 3> block{0, 0, 0};
  std::uint32_t alignment = 0;
  std::uint64_t prefix_bytes = 0;
  std::uint64_t current_k_bytes = 0;
  std::uint64_t current_v_bytes = 0;
  std::uint64_t packed_k_bytes = 0;
  std::uint64_t packed_v_bytes = 0;
  std::uint64_t module_bytes = 0;
  std::array<std::uint8_t, 32> module_sha256{};
};

Status ParseKvPackPayload(const std::uint8_t* data, std::size_t size,
                          KvPackPayloadView* output);
const char* KvPackSymbol() noexcept;

}  // namespace aginfer::internal
